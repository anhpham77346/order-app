...
function Table({ data }: { data: GetAllTableRes }) {
    ...
    function goToDetail() {
        navigate(`${routes.tableDetail}?id=${data.id}`);
    }

    const deleteA = async () => {
        await deleteTable({ tableId: data.id });
